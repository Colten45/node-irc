...
function Client(server, nick, opt) {
    ...
    self.addListener('raw', function(message) {
        ...
        switch (message.command) {
                    ...
                    if (match) {
                        ...
                        switch (param) {
                            ...
                            case 'PREFIX':
                                match = value.match(/\((.*?)\)(.*)/);
                                if (match) {
                                    self.supported.prefixes = match[2];
                                ...
                                break;
                            case 'STATUSMSG':
                                self.supported.statusmsg = value;
                                break;
                            case 'TARGMAX':
                                value.split(',').forEach(function(val) {
                                    val = val.split(':');
                            ...
                            case 'TOPICLEN':
                                ...
                                break;
                            case 'UHNAMES':
                                if (self.opt.uhnames == true) {
                                  self.supported.uhnames = true;
                                }
                                break;
                            case 'NAMESX':
                                if (self.opt.namesx == true) {
                                  self.supported.namesx = true;
                                }
                                break;
                        }
                    }
            ...
            case 'MODE':
                if (self.opt.debug)
                    util.log('MODE: ' + message.args[0] + ' By: ' + message.nick + ' sets mode: ' + message.args[1]);
            ...
            case 'rpl_endofmotd':
            case 'err_nomotd':
                if (self.opt.namesx == true && self.supported.namesx == true) {
                  self.send('PROTOCTL NAMESX');
                }
                if (self.opt.uhnames == true && self.supported.uhnames == true) {
            ...
            case 'rpl_namreply':
                ...
                var regexmatchprefix = new RegExp('^([' + self.supported.statusmsg + ']*)(.*)$');
                if (self.opt.uhnames && self.supported.uhnames)
                  regexmatchprefix = new RegExp('^([' + self.supported.statusmsg + ']*)(.*)(?:(?:!)(.*))$');
                if (channel) {
                    users.forEach(function(user) {
                        var match = user.match(regexmatchprefix);
                        if (match) {
            ...
            case 'JOIN':
                ...
                if (self.nick == message.nick) {
                    ...
                    if (channel && channel.users) {
                      // TODO BLAH WHAT AM I DOING PUT '' BACK!
            ...
            case 'PRIVMSG':
                ...
                if (self.supported.channel.types.indexOf(to.charAt(0)) !== -1) {
                    message['modes'] = '';
                    message['modes'] = self.chanData(to).users[message.nick];
                    self.emit('message#', from, to, text, message);
                    self.emit('message' + to, from, text, message);
                    if (self.opt.debug)
                        util.log('GOT MESSAGE in ' + to + ' from <' + from + '>: ' + text);
...
Client.prototype.send = function(command) {
    ...
    if (args.length == 1) {
        args = Array.prototype.slice.call(arguments[0].split(" "));
